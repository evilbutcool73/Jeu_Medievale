class MapZoom:
    def __init__(self, canvas, map_manager):
        ...
        self.cell_size = map_manager.cell_size
    ...
    def mettre_a_jour_zoom(self, delta):
        ...
        nouvelle_taille = self.cell_size + (facteur_zoom if delta > 0 else -facteur_zoom)
        nouvelle_taille = max(20, min(nouvelle_taille, 100))  # Limiter entre 20 et 100

        if nouvelle_taille != self.cell_size:
            self.cell_size = nouvelle_taille
            self.map_manager.cell_size = nouvelle_taille  # Mettre à jour la taille dans le map_manager

            # Recalculer le nombre de colonnes et lignes visibles
            self.map_manager.nb_colonnes_visibles = self.canvas.winfo_width() // self.cell_size
            self.map_manager.nb_lignes_visibles = self.canvas.winfo_height() // self.cell_size
